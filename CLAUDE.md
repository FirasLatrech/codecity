...
- `src/trailer.js` — one-click cinematic trailer (K / film button): a scripted 5-shot camera sequence (hero push-in → street skim → timelapse growth → fast orbit → wide outro, 26.5s) recorded to a 1080p WebM with captions. captureStream on the WebGL canvas can't see DOM, so each frame the GL frame + captions are drawn into a 1920×1080 2D canvas and THAT is recorded (`composite()` runs after render, same tick, so no preserveDrawingBuffer needed). No audio track — user adds music in an editor. Forces max quality + hides floating faces/signs/car for the take, restores on finish. Esc/K cancels.
